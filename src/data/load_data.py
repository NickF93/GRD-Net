  ...
  if (load_masks):
    def gen_mask_path(path):
      if (os.path.exists(path)):
        ...
        path = '.'.join(path.replace(path.split('/')[-3], mask_dir).split('.')[0:-1]) + '_' + str(mask_ext) + '.' + file_ext
        #path = '.'.join(path.replace(path.split('/')[-3], mask_dir).split('.')[0:-1]) + '_mask.' + str(mask_ext)
        #path = '.'.join(path.replace(path.split('/')[-3], mask_dir).split('.')[0:-1]) + '.' + str(mask_ext)
        if (os.path.exists(path)):
          return path
        else:
          return '' 
