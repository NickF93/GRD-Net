...
class Trainer:
    ...
    def losses_step(
        ...
        # This is true only inside the M (mask of ground truth generated by Perlin Noise)
        # X' = X * (1-beta) + N * beta
        # |X * (1-beta) - X'| = N * beta
        # M' will tend to be close to M so (M' -> M):
        # |(M * X * (1-beta)) - M' * X'| -> N * beta
        # So: (|(M * X * (1-beta)) - M' * X'| - N * beta) ^ 2 -> 0
        noise_loss = tf.reduce_mean(
            (
                (tf.math.abs(((xr * mr) * (1 - beta)) - (xf * mf))) -
                (n * beta)
            ) ** 2.0
        )
    ...
    def train_loop(self, epoch: int, emanager: Optional[ExperimentManager] = None):
        ...
        with tqdm(iterable=self.ds_training_path, leave=True, desc=f'Train {epoch + 1}', unit='batch') as pbar:
            for idx, inputs in enumerate(pbar):
                ...
                averaged_loss_str_dict['lr'] = f'{float(self.generator_optimizer.learning_rate.numpy()):.4f}'
                averaged_loss_dict['lr'] = (float(self.generator_optimizer.learning_rate.numpy()))
